...
if(isset($_GET['pending']))
	{
            $eid=$_GET['pending'];
            $status="1";
            $sql = $dbh->prepare("UPDATE Admin SET Status=:status WHERE  id=:eid");
...
if(isset($_GET['approved']))
	{
                ...
                $status=0;
                $sql =$dbh->prepare("UPDATE Admin SET Status=:status WHERE  id=:aeid");
                $sql -> bindParam(':status',$status, PDO::PARAM_STR);
                $sql-> bindParam(':aeid',$aeid, PDO::PARAM_STR);
                if($sql->execute()){
                    $_SESSION['status']="User Put In Pending State Successfully";
                    $_SESSION['status_code']="success";
                    header('Location:../Approvement.php');
...
if(isset($_REQUEST['del']))
	{
                $did=intval($_GET['del']);
                $sql = $dbh->prepare("delete from Admin WHERE  id=:did");
